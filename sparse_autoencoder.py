...
class SparseAutoencoder(HookedRootModule):
    ...
    def __init__(
        ...
        # multilayer SAE hack -- make d_in the sum of all inputs' d_ins
        if self.cfg.is_multilayer:
            self.d_in = len(self.cfg.multilayer_layers)*self.d_in
            self.d_out = len(self.cfg.multilayer_layers)*self.d_out
        ...
        if not self.cfg.is_high_rank:
            ...
            # TODO: does kaiming uniform work properly with rank-3 tensors?
            self.W_dec = nn.Parameter(
                torch.nn.init.kaiming_uniform_(
                    torch.empty(self.d_sae, self.cfg.high_rank_r, self.d_out, dtype=self.dtype, device=self.device)
                )
            )
        ...
        if self.cfg.is_high_rank:
            self.W_enc_highrank = nn.Parameter(
                torch.nn.init.kaiming_uniform_(
                    torch.empty(self.d_in, self.cfg.high_rank_r, self.d_sae, dtype=self.dtype, device=self.device)
                )
            )
            self.b_enc_highrank = nn.Parameter(
                torch.zeros(self.cfg.high_rank_r, self.d_sae, dtype=self.dtype, device=self.device)
            )

        if self.cfg.is_reglu:
            self.W_enc_reglu = nn.Parameter(
                torch.nn.init.kaiming_uniform_(
                    torch.empty(self.d_in, self.d_sae, dtype=self.dtype, device=self.device)
                )   
            )
            self.b_enc_reglu = nn.Parameter(
                torch.zeros(self.d_sae, dtype=self.dtype, device=self.device)
            )
        ...
        #print('aux_k:', self.cfg.aux_k)
        print("Using reglu: " + str(self.cfg.is_reglu))
    ...
    def forward(self, x, dead_neuron_mask = None, mse_target=None):
        ...
        if self.cfg.is_reglu:
            gated_vals = einops.einsum(sae_in, self.W_enc_reglu,
                '... d_in, d_in d_sae -> ... d_sae'
            ) + self.b_enc_reglu
            hidden_pre = torch.nn.functional.relu(hidden_pre)
            hidden_pre = hidden_pre * gated_vals

        if self.cfg.k_sparse is None:
            if self.cfg.is_reglu:
                feature_acts = hidden_pre
            else:
                feature_acts = self.hook_hidden_post(torch.nn.functional.relu(hidden_pre))
        else:
            feature_acts = torch.zeros(hidden_pre.shape, dtype=self.cfg.dtype).to(device=self.cfg.device)
            vals, idxs = torch.topk(hidden_pre, k=self.cfg.k_sparse)
            feature_acts = torch.scatter(feature_acts, -1, idxs, vals)            
        ...
        if not self.cfg.is_high_rank:
            ...
            cur_highrank_W_enc = self.W_enc_highrank[:, :, idxs].reshape(-1, self.d_in, self.cfg.high_rank_r, self.cfg.k_sparse)
            high_rank_mid = einops.einsum(
                sae_in,
                cur_highrank_W_enc,
                "... d_in, ... d_in r k -> ... r k",
            )
            #print(high_rank_mid.shape)
            #print(self.b_enc_highrank[:, idxs].shape)
            high_rank_mid = high_rank_mid + self.b_enc_highrank[:, idxs].reshape(-1, self.cfg.high_rank_r, self.cfg.k_sparse)

            cur_highrank_W_dec = self.W_dec[idxs, :, :].reshape(-1, self.cfg.k_sparse, self.cfg.high_rank_r, self.d_out)
            high_rank_out = einops.einsum(
                high_rank_mid,
                cur_highrank_W_dec,
                "... r k, ... k r d_out -> ... d_out k",
            )
            sae_out = high_rank_out.sum(dim=-1)
        
        if self.cfg.is_transcoder:
            sae_out = self.hook_sae_out(sae_out + self.b_dec_out)
        else:
            sae_out = self.hook_sae_out(sae_out + self.b_dec)
        ...
        elif self.cfg.aux_k is not None and self.training and dead_neuron_mask.sum() > 0:
            # 1.
            residual = x - sae_out
            l2_norm_residual = torch.norm(residual, dim=-1)
            
            # 2.
            aux_acts = hidden_pre[:, dead_neuron_mask]
            neuron_idxs = torch.arange(len(dead_neuron_mask)).to(device=self.device)
            dead_neuron_idxs = neuron_idxs[dead_neuron_mask]
            top_dead_vals, top_dead_idxs = torch.topk(aux_acts, k=min(self.cfg.aux_k, aux_acts.shape[-1]))
            top_dead_idxs = dead_neuron_idxs[top_dead_idxs]

            ghost_acts = torch.zeros(hidden_pre.shape, dtype=self.cfg.dtype).to(device=self.cfg.device)
            feature_acts = torch.scatter(ghost_acts, -1, top_dead_idxs, top_dead_vals) 
            ghost_out = einops.einsum(
                ghost_acts,
                self.W_dec,
                "... d_sae, d_sae d_out -> ... d_out",
            )
        ...
        if self.cfg.k_sparse is None:
            ...
            mse_loss_ghost_resid = mse_loss_ghost_resid * self.cfg.aux_k_alpha
