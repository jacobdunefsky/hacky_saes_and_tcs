...
class ActivationsStore:
    ...
    def __init__(
        ...
        if self.cfg.is_multilayer:
            self.multilayer_names = [f'blocks.{layer}.{self.cfg.multilayer_hook_point_str}' for layer in self.cfg.multilayer_layers]
        self.model = model
            ...
            if not self.cfg.is_transcoder:
                if not self.cfg.is_multilayer:
                    ...
                    cache = self.model.run_with_cache(
                        batch_tokens,
                        names_filter=self.multilayer_names,
                        stop_at_layer=max(self.cfg.multilayer_layers)+1
                    )[1]
                    activations = torch.concatenate([cache[name] for name in self.multilayer_names], dim=-1)
    ...
    def get_buffer(self, n_batches_in_buffer):
        ...
        batch_size = self.cfg.store_batch_size
        d_in = self.cfg.d_in
        # for multilayer SAEs, d_in is the sum of all d_ins across layers
        if self.cfg.is_multilayer: d_in = d_in * len(self.cfg.multilayer_layers)
